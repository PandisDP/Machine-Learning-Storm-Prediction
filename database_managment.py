...
def dataset_managment(case):
    ...
    if case==1:
        ...
        db.generate_DataSet(config['params_model_DSCOVR'],'bt','db_Plasma_2016-2023.csv')
    if case==2:  
    ...
    if case==3:
        ...
        dc= DataBase_CASSIOPE(config['output_filename_cass'],config['settings_target'])
        dc.generate_DataSet(int(config['settings_target_value']),config['params_cass'])
    if case==4:  
        #4.0 Integrations CASSIOPE and DSCOVR Datasets
        di= DATABASE_integration()
        di.dataset_generation_DS_CASS(config['PATH_FILE_CASS'],config['PATH_FILE'],config['PATH_OUTPUT'])
    else:
        pass    